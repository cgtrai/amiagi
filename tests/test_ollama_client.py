...
def test_chat_falls_back_to_alternate_model_after_runner_stop(monkeypatch) -> None:
    seen_attempts: list[tuple[str, int]] = []

    def fake_post(self, path, payload):  # type: ignore[no-untyped-def]
        seen_attempts.append((str(payload["model"]), int(payload["options"]["num_ctx"])))
        if str(payload["model"]) == "unstable-model":
            raise OllamaClientError(
                'HTTP 500: {"error":"model runner has unexpectedly stopped, this may be due to resource limitations or an internal error, check ollama server logs for details"}'
            )
        return {"message": {"content": "ok"}}
    ...
    assert client.model == "stable-model"


def test_chat_logs_fallback_model_retry(monkeypatch) -> None:
    logger = DummyLogger()
    ...
    assert response == "ok"
    assert any(record["action"] == "model.chat.retry.fallback_model" for record in logger.records)
