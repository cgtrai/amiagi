...
class WorkflowDag extends HTMLElement {
  ...
  _render() {
    ...
    const NODE_W = 130, NODE_H = 50, PAD_X = 40, PAD_Y = 30;
    ...
    this._nodes.forEach((node) => {
      ...
      label.setAttribute("y", pos.y + NODE_H / 2 - 6);
      label.textContent = node.label || node.id;
      g.appendChild(label);

      const status = document.createElementNS(ns, "text");
      status.setAttribute("class", "node-status");
      status.setAttribute("x", pos.x + NODE_W / 2);
      status.setAttribute("y", pos.y + NODE_H / 2 + 10);
